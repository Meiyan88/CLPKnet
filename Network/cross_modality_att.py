...
import torch
from Network.layers import get_config, Mlp
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        return embeddings # b, n_patches, c
...
class AttentionCrossModal(nn.Module):
    ...
    def forward(self, hidden_states, kv):   # # b (w h) c & b m*s c
...
class TokenLearner(nn.Module):
    def __init__(self, in_channels, S):
        ...
        super().__init__()
        self.token_conv = nn.Conv2d(in_channels=in_channels, out_channels=S, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        selected = self.token_conv(x)
        selected = rearrange(selected, "b s w h -> b s (w h) 1")
        selected = torch.sigmoid(selected)

        x = rearrange(x, "b c w h -> b 1 (w h) c")

        out = (x * selected).mean(dim=2)    #  b 1 (w h) c * b s (w h) 1 -> b s c
        return out

class CrossModalityFusion(nn.Module):
    def __init__(self, model_num, in_channels,
                 hidden_size,
                 img_size, mlp_size=256,patch_size=(1,1),
                 token_mixer_size=32,
                 token_learner=False):
        super().__init__()
        ...
        self.patch_num = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])
        self.token_learner = token_learner
        if token_learner:
            self.token_mixer = TokenLearner(in_channels=hidden_size, S = token_mixer_size)
        else :
            self.token_mixer = nn.Linear(self.patch_num, token_mixer_size)
    ...
    def forward(self, q, kv):
        ...
        for i in range(self.model_num):
            ...
            x = self.embeddings[i](kv[:, i])    # b n_patches c

            if self.token_learner:
                x = rearrange(x, "b (w h) c -> b c w h", w=self.img_size[0] // self.patch_size[0], h=self.img_size[1] // self.patch_size[1])
                x = self.token_mixer(x) # b, s, c

            else :
                x = x.transpose(-1, -2) # b n_patches c -> b c n_patches
                x = self.token_mixer(x) # b c n_patches -> b c s
                x = x.transpose(-1, -2) # b c s -> b s c

            embed_x.append(x)

        embed_x = torch.cat(embed_x, dim=1) # m [b s c] -> b m*s c
        batch_size = embed_x.shape[0]

        corss_out = self.cross_attention(q, embed_x) # b (w h) c & b m*s c
        corss_out = corss_out.transpose(-1, -2)
        corss_out = corss_out.view((batch_size, self.hidden_size, self.img_size[0], self.img_size[1]))
