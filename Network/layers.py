...
class PostionEmbedding(nn.Module):
    def __init__(self, config, types=0):
        ...
        if types == 0:
            self.position_embeddings = nn.Parameter(torch.zeros(1, (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1]), config.hidden_size))
        elif types == 1:
            self.position_embeddings = nn.Parameter(torch.zeros(1, (img_size[1] // patch_size[1]), config.hidden_size))
        elif types == 2:
