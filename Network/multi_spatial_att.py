...
from Network.layers import Attention, ACT2FN, PostionEmbedding, get_config, Mlp
...
def get_relative_distances(window_size):
    indices = torch.tensor(np.array([[x, y] for x in range(window_size[0]) for y in range(window_size[1])]),dtype=torch.float)
...
class WindowAttention(nn.Module):
    def __init__(self, dim, heads, head_dim, window_size, relative_pos_embedding):
        ...
        if self.relative_pos_embedding:
            self.relative_indices = get_relative_distances(window_size).long()
            min_indice = self.relative_indices.min()
            self.relative_indices += (-min_indice)
            max_indice = self.relative_indices.max().item()
...
class MultiAttention(nn.Module):
    def __init__(self, config, is_position=False):
        ...
        if is_position:
            self.pos_embedding_1 = PostionEmbedding(config, types=0)

    def forward(self, x):
        batch_size, hidden_size, W, H = x.shape

        x_1 = rearrange(x, "b c w h -> b (w h) c")    # MHA_xy
        x_3 = rearrange(x, "b c w h -> b w h c") # MHA_w
        # x_3 = x.permute(0, 2, 3, 4, 1) # b c d w h -> b d w h c # MHA_w
        ...
        x_3 = rearrange(x_3, "b w h c -> b (w h) c", w=W, h=H)



        return x_1 + x_3    # b (w h) c
...
class MultiAttBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(-1, -2) # b (w h) c -> b c (w h)
        out_size = (self.input_shape[0] // self.config.patch_size[0],
                    self.input_shape[1] // self.config.patch_size[1])
        x = x.view((batch_size, self.config.hidden_size, out_size[0], out_size[1])).contiguous()
...
class MultiSpatialFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.embeddings(x)  # b (m f) w h -> b c n_patches n_patches
        hidden_states = []
