...
# # 3D dataset
path_weight = r''
class MSFEDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train_val_test == 'train':
            ...
            image = []
            for c in range(C):
                image.append(transform.resize(_img[c], output_shape=[96, 96], order=1, mode='constant', clip=False,
                                 preserve_range=True)[np.newaxis, :, :])
            ...
            mask_gt = transform.resize(mask_gt, output_shape=[96, 96], order=0, mode='constant', clip=False,
            ...
            Gt = torch.as_tensor(Gt, dtype=torch.long)
             #loss weight_map
            #self.posRa[index] = self.posRa[index].replace('')
            with open (path_weight,'r') as weights:
                #print('I am here1')
                for weight_i in weights:
                    weight_i = weight_i.strip()
                    weight_i0 = weight_i.split("  ")[0]
                    weight_i1 = weight_i.split("  ")[1]
                    #print(weight_i0,self.posRa[index])
                    if weight_i0 == self.posRa[index]:

                        weight_map = np.ones((1,9))
                        w1 = ast.literal_eval(weight_i1)
                        for i in w1:
                            i1 = i - 1
                            weight_map[0,i1] = 0
                        weight_map = weight_map.reshape(3,3)
            ...
            mask_gt = _mask_posra[x_min:x_max + 1, y_min:y_max + 1]
            mask_union = transform.resize(mask_union, output_shape=[96, 96], order=0, mode='constant', clip=False,
                                          preserve_range=True, anti_aliasing=False)
            mask_gt = transform.resize(mask_gt, output_shape=[96, 96], order=0, mode='constant', clip=False,
                                       preserve_range=True, anti_aliasing=False)
            Gt = np.zeros((3, 3))
...
if __name__ == "__main__":
    ...
    from Operation import Preparation

    args = Preparation.addArgs()
    train_data = os.path.join(config["train_path"], "split0", "train1.txt")
    val_data = os.path.join(config["val_path"], "split0", "test1.txt")
    test_data = config["independent_dataset"]

    train_dataset = MSFEDataset(args, config, root=train_data, train_val_test='train')
    val_dataset = MSFEDataset(args, config, root=val_data, train_val_test='val')
    test_dataset = MSFEDataset(args, config, root=test_data, train_val_test='test')

    train_dataset_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1, shuffle=False)
    val_dataset_loader = torch.utils.data.DataLoader(val_dataset, batch_size=1, shuffle=False)
