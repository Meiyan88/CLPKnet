...
class MSFEDataset_4(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train_val_test == 'train':
            ...
            image = []
            for c in range(C):
                image.append(transform.resize(_img[c], output_shape=[64, 64], order=1, mode='constant', clip=False,
                                              preserve_range=True)[np.newaxis, :, :])
            ...
            mask_ca = transform.resize(_ca, output_shape=[64, 64], order=0, mode='constant', clip=False,
                                       preserve_range=True, anti_aliasing=False)
            mask_gt = transform.resize(mask_gt, output_shape=[64, 64], order=0, mode='constant', clip=False,
                                       preserve_range=True, anti_aliasing=False)
            Gt = np.zeros((2, 2))
            ...
            mask_union = mask_union[x_min:x_max + 1, y_min:y_max + 1]
            mask_gt = _mask_posra[x_min:x_max + 1, y_min:y_max + 1]
            mask_union = transform.resize(mask_union, output_shape=[64, 64], order=0, mode='constant', clip=False,
                                          preserve_range=True, anti_aliasing=False)
            mask_gt = transform.resize(mask_gt, output_shape=[64, 64], order=0, mode='constant', clip=False,
                                       preserve_range=True, anti_aliasing=False)
            Gt = np.zeros((2, 2))
